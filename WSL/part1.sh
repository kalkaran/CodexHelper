...
Install notes:
  - apt-get installs ShellCheck/shfmt as machine-level CLI tools on Ubuntu WSL.
...
For this PHP/static HTML/CSS/JS repo:
  ...
  # PHP:
  # If composer is missing on Ubuntu/WSL, install it first with: sudo apt-get install composer
  ...
  # Security scanner:
  # If semgrep is missing, install it with: pipx install semgrep
